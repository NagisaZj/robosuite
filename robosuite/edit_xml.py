...
class MujocoXMLModel(MujocoXML, MujocoModel):
    ...
    def __init__(self, fname, idn=0):
        ...
        if macros.USING_INSTANCE_RANDOMIZATION:
            tex_element, mat_element, _, used = add_material(root=self.worldbody, naming_prefix='')
...
if __name__=="__main__":
    file_names = ['./models/assets/robots/panda/robot.xml','./models/assets/robots/sawyer/robot.xml','./models/assets/robots/iiwa/robot.xml','./models/assets/robots/kinova3/robot.xml']
    nicknames=['panda','sawyer','iiwa','kinova3']
    file_names = ['./models/assets/robots/kinova3/robot.xml']
    nicknames = ['kinova3']
    ranges = [[0.5]*10,[0.6]*10,[0.7]*10,[0.8]*10,[0.9]*10,[1.0]*10,[1.1]*10,[1.2]*10,[1.3]*10,[1.4]*10,[1.5]*10,[0.5]*5+[1.5]*5,[1.5]*5+[0.5]*5,[0.7]*5+[1.3]*5,[1.3]*5+[0.7]*5,[0.9]*5+[1.1]*5,[1.1]*5+[0.9]*5]
    ranges = [[0.5] * 8, [0.6] * 8, [0.7] * 8, [0.8] * 8, [0.9] * 8, [1.0] * 8, [1.1] * 8, [1.2] * 8,
              [1.3] * 8, [1.4] * 8, [1.5] * 8, [0.5] * 4 + [1.5] * 4, [1.5] * 4 + [0.5] * 4, [0.7] * 4 + [1.3] * 4,
              [1.3] * 4 + [0.7] * 4, [0.9] * 4 + [1.1] * 4, [1.1] * 4 + [0.9] * 4]
    if not os.path.exists('./my_xmls'):
        os.makedirs('./my_xmls',exist_ok=1)
    for num,f in enumerate(file_names):
        # print(num,f)
        if not os.path.exists('./my_xmls/'+nicknames[num]):
            os.makedirs('./my_xmls/'+nicknames[num], exist_ok=1)
        for num2 in range(len(ranges)):
            model = MujocoXMLModel(f)
            for num3, mass_items in enumerate(model.root.iter('inertial')):
                mass_items.attrib['mass'] = str(float(mass_items.attrib['mass']) * ranges[num2][num3])

            model.save_model('./my_xmls/'+nicknames[num]+'/'+str(ranges[num2])+'.xml')
